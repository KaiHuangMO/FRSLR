...
class MLSOL():
    ...
    def generate_instance(self, x_seed, y_seed, T_seed, x_reference, y_reference, T_reference):
        ...
        y_synthetic = np.array(list(map(int, y_synthetic)))
        return x_synthetic, y_synthetic

    def fit_resample(self, X, y, gen_num = 0):
        y_new_ori = list(copy.deepcopy(y))
        ...
        X_synthesized = np.zeros((gen_num, X.shape[1]))
        y_synthesized = np.zeros((gen_num, y.shape[1]), dtype=int)
        ...
        for i in trange(gen_num):
            ...
            reference_index = indices[seed_index][random.randint(1, self.k)]  # SMOTE依赖的另一个数据
            ...
            gen_num -= 1
            counter += 1
            min_set.add(seed_index)
            y_new_ori.append(y[seed_index, :])

        print ('MLSOL GEN ' + str(len(X_synthesized)))

        X_aug = np.concatenate((X, np.flip(X_synthesized, axis=0)), 0)
        y_aug = np.concatenate((y, np.flip(y_synthesized, axis=0)), 0)

        return X_aug, y_aug, y_new_ori[y.shape[0]:], min_set
